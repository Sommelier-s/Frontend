...
const Register = () => {
  ...
  const validateForm = () => {
    const { name, lastName, email, phoneNumber, password, confirmPassword, acceptTerms } = formData;
    const errors = {};
    ...
    if (!lastName.match(/^[a-zA-Z ]*$/)) {
      errors.lastName = 'Apellido inválido. Solo se permiten letras y espacios.';
    }
    ...
    if (!phoneNumber.match(/^\d+$/)) {
      errors.phoneNumber = 'Número de teléfono inválido. Solo se permiten dígitos.';
    }

    if (password.length < 6 || !/\d/.test(password) || !/[A-Z]/.test(password)) {
      errors.password = 'Contraseña inválida. Debe tener al menos 6 caracteres, un número y una letra mayúscula.';
    }
  ...
  const validateField = (fieldName, value) => {
    ...
    switch (fieldName) {
      ...
      case 'lastName':
        if (!value.match(/^[a-zA-Z ]*$/)) {
          fieldErrors.lastName = 'Apellido inválido. Solo se permiten letras y espacios.';
        }
        break;
      case 'email':
      ...
      case 'phoneNumber':
        if (!value.match(/^\d+$/)) {
          fieldErrors.phoneNumber = 'Número de teléfono inválido. Solo se permiten dígitos.';
        }
        break;
      case 'password':
        if (value.length < 6 || !/\d/.test(value) || !/[A-Z]/.test(value)) {
          fieldErrors.password = 'Contraseña inválida. Debe tener al menos 6 caracteres, un número y una letra mayúscula.';
        }
        break;
      ...
      <Grid item xs={12} sm={8} md={6}>
        <Paper style={paperStyle}>
          <Grid align="center">
            ...
            <Typography variant="subtitle2">Complete los campos para crear su cuenta</Typography>
          </Grid>
