...
export default function ProductMonth({
	...
	const handleProductMonthTwo = async (event) => {
		...
		if (!productMonth) {
			if (product.length === 0) {
				if (!graduation) {
					const { data } = await axios.put(`/wine/${id}/?userId=${user.id}`, {
						is_product_month: true,
					});
				} else {
					const { data } = await axios.put(`/liquor/${id}/?userId=${user.id}`, {
						is_product_month: true,
					});
				}
					...
					if (response) {
						...
						if (!graduation) {
							axios
								...
								.catch((error) => {
									console.log(
										'wine, no se pudo cargar el nuevo producto:',
										error,
									);
								});
						} else {
							axios
								.put(`/liquor/${id}/?userId=${user.id}`, {
									is_product_month: true,
								})
								.then((response) => {
								...
								.catch((error) => {
									console.log(
										'liquor, no se pudo cargar el nuevo producto:',
										error,
									);
								});
						}
						...
						if (product[0].graduation) {
							axios
								...
								.catch((error) => {
									console.log(
										'liquor, no se pudo borrar el viejo producto:',
										error,
									);
								});
						} else {
							axios
								.put(`/wine/${product[0].id}/?userId=${user.id}`, {
									is_product_month: false,
								})
								.then((response) => {
								...
								.catch((error) => {
									console.log(
										'wine, no se pudo borrar el viejo producto:',
										error,
									);
								});
						}
						...
						<IconButton onClick={handleProductMonthTwo}>
							<StarBorderIcon
								className={`${styles.icon} ${
									productMonth ? styles.productMonth : ''
								}`}
							/>
						</IconButton>
					</div>
